...
export async function jsonResponse(message) {
  return new Response(JSON.stringify({ response: message }), {
    headers: { "content-type": "application/json" },
  });
}

export async function getVehicleIDAndStatusFromVin(accessToken, vin) {
  if (vin === null) {
    throw "No X-Tesla-vin header provided";
  }

  const vehiclesResponse = await teslaRequest(
    accessToken,
    null,
    "GET",
    "/vehicles"
...
export async function startHVAC(accessToken, vehicleID) {
  return await teslaRequest(
    accessToken,
    vehicleID,
    "POST",
...
export async function setTemperature(accessToken, vehicleID, temperature) {
  return await teslaRequest(
    accessToken,
    vehicleID,
    "POST",
...
export async function wakeVehicle(accessToken, vehicleID) {
  const wakeResponse = await teslaRequest(
    accessToken,
    vehicleID,
    "POST",
    "/wake_up"
...
export async function teslaRequest(
  accessToken,
  vehicleID,
  method,
  url,
  ...
  if (vehicleID === null && url === "/vehicles") {
    ...
    fullUrl =
      "https://owner-api.teslamotors.com/api/1/vehicles/" + vehicleID + url;
  }
  ...
  while (response.status === 408) {
    ...
    console.warn(
      `[Attempt: ${retryCount}/${retryLimit}] Car is sleeping, trying again in ${retryMs}ms`
    );
  ...
  if (response.status === 200) {
    ...
    throw (
      `Invalid response from ${fullUrl}: [${response.status}] ` +
      (await response.text())
    );
  }
}

export async function performRequest(request, body) {
